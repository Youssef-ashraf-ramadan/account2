...
import { FaPaperclip, FaTimes, FaPlus, FaArrowRight } from 'react-icons/fa';
import {
  addReceiptVoucher,
  getPostingAccounts,
  clearError,
...
const AddReceiptVoucher = () => {
  ...
  const { postingAccounts, isLoading, error, success } = useSelector((state) => state.auth);
  ...
  const handleSubmit = (event) => {
    ...
    if (!form.account_id) {
      toast.error('برجاء اختيار الحساب المستلم', { rtl: true });
      return;
    }
    ...
    if (normalizedDetails.length === 0 || normalizedDetails.some((detail) => !detail.account_id || detail.amount <= 0)) {
      toast.error('برجاء استكمال تفاصيل الإيصال بشكل صحيح', { rtl: true });
      return;
    }
    ...
    dispatch(addReceiptVoucher(payload));
  };
  ...
  return (
    <div style={pageWrapperStyle}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <h1 style={titleStyle}>إضافة إيصال استلام نقدية</h1>
      </div>
      <form onSubmit={handleSubmit} style={formCardStyle}>
        <div style={formGridStyle}>
          <div>
            <label style={labelStyle}>تاريخ الإيصال <span style={{ color: 'var(--red-color)' }}>*</span></label>
            <input
              type="date"
          ...
          <div>
            <label style={labelStyle}>الحساب المستلم <span style={{ color: 'var(--red-color)' }}>*</span></label>
        ...
        <div style={{ marginTop: '24px' }}>
          <div style={sectionHeaderStyle}>تفاصيل الإيصال</div>
