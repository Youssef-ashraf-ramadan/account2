...
const PaymentVouchers = () => {
  ...
  const { paymentVouchers, paymentVouchersPagination, isLoading, error, success } = useSelector((state) => state.auth);
  ...
  const handleDelete = (id) => {
    if (window.confirm('هل أنت متأكد من حذف سند الصرف؟')) {
      dispatch(deletePaymentVoucher(id));
    }
  };
  ...
  const renderTableBody = () => {
    ...
    if (!paymentVouchers || paymentVouchers.length === 0) {
      return (
        <tr>
          <td colSpan="8" style={emptyStateStyle}>لا توجد سندات صرف متاحة</td>
        </tr>
      );
    }
    return paymentVouchers.map((voucher, idx) => {
      ...
      return (
        <tr
          key={voucher.id}
          style={baseRowStyle}
          onMouseEnter={(ev) => { ev.currentTarget.style.backgroundColor = 'rgba(0, 106, 255, 0.08)'; }}
          onMouseLeave={(ev) => { ev.currentTarget.style.backgroundColor = idx % 2 === 0 ? 'var(--basic-color)' : '#f0f4fa'; }}
        >
          <td style={cellStyle}>{(paymentVouchersPagination?.current_page - 1) * (paymentVouchersPagination?.per_page || 10) + idx + 1}</td>
  ...
  return (
    <div style={pageWrapperStyle}>
      <div style={headerStyle}>
        <h1 style={titleStyle}>سندات الصرف</h1>
        <button
          onClick={() => navigate('/payment-vouchers/add')}
          style={addButtonStyle}
        >
          <FaPlus style={{ color: 'white' }} /> إضافة سند
        </button>
      </div>
      ...
      <div style={cardStyle}>
        ...
        {paymentVouchersPagination && (paymentVouchers?.length || 0) > 0 && paymentVouchersPagination.last_page > 1 && (
          <div style={paginationStyle}>
            {Array.from({ length: paymentVouchersPagination.last_page }, (_, i) => i + 1).map((page) => (
              <button
                key={page}
                onClick={() => setCurrentPage(page)}
                style={{
                  ...paginationButtonStyle,
                  backgroundColor: page === paymentVouchersPagination.current_page ? 'var(--main-color)' : 'var(--basic-color)',
                  color: page === paymentVouchersPagination.current_page ? 'white' : '#182d40',
                  border: page === paymentVouchersPagination.current_page ? 'none' : '1px solid var(--border-color)'
                }}
              >
                {page}
              </button>
